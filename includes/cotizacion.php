<section id="cotizacion">
		<div class="container">
			<div class="col-md-3 cotizacion-intro">
				<h3>Cotización rápida</h3>
				<p class="text-cot1">Solicitar precios y formas de pago de los modelos que más te gusten.</p>
				<h4><i class="fa fa-refresh" aria-hidden="true"></i> 24 hs</h4>
				<p class="text-cot2">Esperamos tu consulta. Obtendrás una respuesta dentro de las 24hs. <span>¡Garantizado!</span></p>

			</div>

			 
			<div class="form-cotizacion col-md-6">
				
				<?php echo do_shortcode('[contact-form-7 id="57" title="Cotizacion - home"]'); ?>
			</div>
			
			<div class="col-md-3">
				<div class="whatsapp">
					<h4>whatsapp</h4>
					<div class="employed">
						<i class="fa fa-whatsapp" aria-hidden="true"></i>
						<div class="face"><img src="<?php echo get_template_directory_uri(); ?>/static/vendedor-oscar.jpg"></div>
						<p class="name">Oscar <small>(En linea)</small></p>
						<p class="cel">(0385) 155147260</p>								
					</div>
					<div class="employed">
						<i class="fa fa-whatsapp" aria-hidden="true"></i>
						<div class="face"><img src="<?php echo get_template_directory_uri(); ?>/static/vendedor-franco.jpg"></div>
						<p class="name">Franco <small>(En linea)</small></p>
						<p class="cel">(0385) 155147254</p>								
					</div>
					<div class="employed">
						<i class="fa fa-whatsapp" aria-hidden="true"></i>
						<div class="face"><img src="<?php echo get_template_directory_uri(); ?>/static/vendedor-matias.jpg"></div>
						<p class="name">Matías <small>(En linea)</small></p>
						<p class="cel">(0385) 155148115</p>								
					</div>
					<p class="tel">
							<i class="fa fa-phone" aria-hidden="true"></i>
							0385 4316472
						</p>
						<!-- <p class="mail">
							<i class="fa fa-envelope" aria-hidden="true"></i> contacto@motogar.com.ar
						</p> -->

				</div>
			</div>
		</div>
	</section>